...
skew160<- apply(sample160, 1 ,skewness)

q10 <- quantile(skew10, c(0.025,0.975))
q40 <- quantile(skew40, c(0.025,0.975))
q160 <- quantile(skew160, c(0.025,0.975))

hist(skew10, freq=TRUE, main="Skewness Distribution of Sample size 10")
abline(v=-1.08, col="red")
abline(v=1.1, col="red")
hist(skew40, freq=TRUE, main="Skewness Distribution of Sample size 40")
abline(v=-0.699, col="red")
abline(v=0.766, col="red")
hist(skew160, freq=TRUE, main="Skewness Distribution of Sample size 160")
abline(v=-0.344, col="red")
abline(v=0.387, col="red")

skewTable <- matrix( c(sd(skew10), sqrt(6 * (10 - 2) / ((10 + 1 ) * (10 + 3))), sd(skew40), sqrt(6 * (40 - 2) / ((40 + 1 ) * (40 + 3))),sd(skew160), sqrt(6 * (160 - 2) / ((160 + 1 ) * (160 + 3)))), ncol=2, byrow=TRUE)

colnames(skewTable) <- c("Sample Standard Dev", "Theoretical Standard Dev")
rownames(skewTable) <- c("Sample size 10", "Sample size 40", "Sample size 160")

skewTable<- as.table(skewTable)
skewTable
```

Kurtosis for a standard normal distribution is 3.0 using the "moments" package *kurtosis()* function. 
...
```{r ecp_kurtosis}
skew10<- apply(sample10, 1 ,kurtosis)
skew40<- apply(sample40, 1 ,kurtosis) 
skew160<- apply(sample160, 1 ,kurtosis)

q10 <- quantile(skew10, c(0.025,0.975))
q40 <- quantile(skew40, c(0.025,0.975))
q160 <- quantile(skew160, c(0.025,0.975))

hist(skew10, freq=TRUE, main="Kurtosis Distribution of Sample size 10")
abline(v=1.51, col="red")
abline(v=4.28, col="red")
hist(skew40, freq=TRUE, main="Kurtosis Distribution of Sample size 40")
abline(v=1.96, col="red")
abline(v=4.53, col="red")
hist(skew160, freq=TRUE, main="Kurtosis Distribution of Sample size 160")
abline(v=2.42, col="red")
abline(v=3.81, col="red")

theo10 <- sqrt(24 * 10 * (10 - 2) * (10 - 3) / ((10 + 1) * (10 + 1) * (10 + 3) * (10 + 5)))
theo40 <- sqrt(24 * 40 * (40 - 2) * (40 - 3) / ((40 + 1) * (40 + 1) * (40 + 3) * (40 + 5)))
theo160<- sqrt(24 * 160 * (160 - 2) * (160 - 3) / ((160 + 1) * (160 + 1) * (160 + 3) * (160 + 5)))

skewTable <- matrix(c(sd(skew10), theo10, sd(skew40), theo40, sd(skew160), theo160), ncol=2, byrow=TRUE)
...
*** As the size of the samples increase, the variability decreases for both the Skewness and kurtosis. The frequency of kurtosis gets closer to 3 and the frequency of skewness gets closer to 0. Based on the charts, Skewness values appear to be normally distrbuted, while the kurtosis values appear to be right skewed. with most of the values focusing between 2 and 3 in all three charts.***

-----

quartile calculations using random samples of different sizes from the standard normal distribution.  The quartiles for the standard normal distribution are (-0.6745, 0.0 and +0.6745), obtainable by *qnorm(c(0.25, 0.5, 0.75), mean = 0, sd = 1, lower.tail = TRUE)*.  This is illustrated below.
...
tiles<- c(0.25, 0.5, 0.75)

set.seed(127)
s1<-rnorm(25, mean = 0, sd = 1)
q12<-quantile(s1, tiles, type = 2)
q17<-quantile(s1, tiles, type = 7)

set.seed(127)
s2<-rnorm(50, mean = 0, sd = 1)
q22<-quantile(s2, tiles, type = 2)
q27<-quantile(s2, tiles, type = 7)

set.seed(127)
s3<-rnorm(100, mean = 0, sd = 1)
q32<-quantile(s3, tiles, type = 2)
q37<-quantile(s3, tiles, type = 7)

quants<-matrix(c(q12, q17, q22, q27, q32, q37),ncol=3, byrow=TRUE)

colnames(quants) <- c("0.25", "0.5", "0.75")
rownames(quants) <- c("Sample size 25 type 2","Sample size 25 type 7","Sample size 50 type 2","Sample size 50 type 7","Sample size 100 type 2","Sample size 100 type 7")

quantsTable<- as.table(quants)
quantsTable
...
for (k in seq(from = 25, to = 5000, by = 25)){
  x <- rnorm(k, mean = 0, sd = 1)
  quant <- rbind(quant, quantile(x, probs = 0.25, type = 7))
}
size <- seq(from = 25, to = 5000, by = 25)
plot(size, quant, main = "1st Quantile versus Sample Size", xlab = "Sample Size", ylab = "1st Quantile", ylim = c(-0.4,-0.9))


quant <- numeric(0)
for (k in seq(from = 25, to = 5000, by = 25)){
  x <- rnorm(k, mean = 0, sd = 1)
  quant <- rbind(quant, quantile(x, probs = 0.75, type = 7))
}
size <- seq(from = 25, to = 5000, by = 25)
plot(size, quant, main = "3rd Quantile versus Sample Size", xlab = "Sample Size", ylab = "3rd Quantile", ylim = c(0.4,0.9))

```



***For the first quartile, as the size of the sample increases, the expected values converge between -0.6 and -0.7. For the third quartile, as the size of the sample increases, the expected values converge between 0.6 and 0.7. Based on the scatterplots, it appears that these quartiles converge at different rates. 1st quartile values begin to converge between -0.6 and -0.7 as the sample size  gets larger than around 2000 observations. 3rd quartile values begin to converge between 0.6 and 0.7 as the sample size gets larger than around 1000 observations. This makes me nervous about using the minimum sample size of 30 needed to assume a normal distribution under the central limit theorem, as the variability in quartile values is very high with such a low sample size.   ***
...
Compute the confidence interval for the population variance using the traditional chi-square method which assumes normality. You will need to add code calculating a 95% confidence interval for the sample variance estimate, based on a chi-square distribution. This assumes normality.
...
quantile.sample = function(x){
  sample<-sample(x, 50, replace=TRUE)
  v<- var(sample)
  c(v)
  }

Bigsample<- replicate(10000, quantile.sample(mag), simplify="array")

hist(Bigsample, freq=TRUE, main="10000 samples variances of mag")
quant <-quantile(Bigsample, tiles, type = 7)
quant

quantsConfidence<-matrix(c(var.conf.int(mag), quant),ncol=2, byrow=TRUE)

colnames(quantsConfidence) <- c("0.025", "0.975")
rownames(quantsConfidence) <- c("chi-squared confidence interval","bootstrap confidence interval")

quantsTable<- as.table(quantsConfidence)
quantsTable
```
...
***Answer: The confidence interval produced by the traditional method yields variances which are higher than the one produced by bootstrapping. If I have access to a service like R, where the calculation to perform the bootstapping takes seconds, I will use bootstrapping. The traditional method is useful if I cannot take that many samples at once.***
...
The user-defined function is passed to *boot()* with "mag" along with the number of samples to be drawn with replacement; here, 10000. The resulting object will be a list. The element "t" of this list is the vector of sample variances. This vector may be used to estimate the relevant quantiles for our interval.
...
bootvar<-boot(data= mag, statistic= f, R=10000)
quant <-quantile(bootvar$t, tiles, type = 7)
quant
...
ci<-boot.ci(bootvar,conf=0.95,type="perc")
ci
...
The data frame contains test results for 49 students on two standardized tests. Each student took both tests.
...
```{r correlation}
corr<- cov(test$testA, test$testB)/sqrt(var(test$testA)*var(test$testB))
corr
...
cor.test(test$testA, test$testB, method= "pearson", conf.level=.95)
...
Plot a histogram and report a two-sided 95% confidence interval. 

```{r bootstrapCorrelation}
set.seed(123)
tiles<- c(0.025, 0.975)
samplecorr <- numeric(0)

for (k in seq(from = 1, to = 10000, by = 1)){
sampletest<-test[sample.int(49, size=49, replace=TRUE),]
samplecorr <- rbind(samplecorr, cor(sampletest[,1],sampletest[,2], method="pearson"))
k+1
}

samplecorrquant<-quantile(samplecorr, tiles, type=7)
samplecorrquant

hist(samplecorr, freq=TRUE, main="10000 samples variances of test correlation coefficient")
abline(v=0.916,col="red")
abline(v=0.969,col="red")
```

Bootstrapping can also be used to arrive at confidence intervals for regression coefficients. Using the current data frame, rows of "test" are randomly sampled with replacement. Each sample is passed to *lm()* and the coefficients extracted. Section 9.5 of Chihara and Hesterberg give an example R script showing how this may be accomplished.
...
cor.boot[i] <- cor(Test.boot$testA, Test.boot$testB)
TestBoot.lm <- lm(testA ~ testB, data = Test.boot)
alpha.boot[i] <- coef(TestBoot.lm)[1] # new intercept
beta.boot[i] <- coef(TestBoot.lm)[2] # new slope
}

alphaquant<-quantile(alpha.boot, tiles, type=7)
alphaquant

betaquant<-quantile(beta.boot, tiles, type=7)
betaquant

hist(alpha.boot, freq=TRUE, main="10000 samples slopes")
abline(v=-20.495, col="red")
abline(v=-8.785, col="red")

hist(beta.boot, freq=TRUE, main="10000 samples intercepts")
abline(v=1.1557, col="red")
abline(v=1.38, col="red")

plot(beta.boot, alpha.boot, main = "slope versus intercept", xlab = "Slope", ylab = "Intercept")
...
***As the slope increases, the intercept value begins to decrease. There appears to be  linear relationship between slope and intercept values   ***
